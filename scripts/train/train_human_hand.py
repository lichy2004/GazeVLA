...
def load_from_pretrain(model: torch.nn.Module, config: _config.TrainConfig):
    ...
    projection_bases = ["action_in_proj", "action_out_proj", "state_proj"]
    skipped_keys = []
    ...
    model_to_load.load_state_dict(ckpt_tensors, strict=False)       # missing = 'paligemma_with_expert.paligemma.model.language_model.embed_tokens.weight'

    if skipped_keys:
        logging.info(
            "Skipped loading mismatched projection params: \n%s",
            "\n".join(skipped_keys),
        )
...
class TimingLogger:
    ...
    def _print_detailed_step_timing(self, step):
        ...
        if timing_info:
            logging.info(f"⏱️  Step {step} Real-time Timing: {', '.join(timing_info)}")
