...
def build_train_datasets(config: _config.TrainConfig):
    # Use the unified data loader with PyTorch framework
    data_loader = _data.create_data_loader(config, framework="pytorch", shuffle=True)
    return data_loader, data_loader.data_config()


def build_eval_dataset(config: _config.TrainConfig, data_config_factory, eval_batch_size=None):
    eval_config = dataclasses.replace(config, data=data_config_factory, batch_size=eval_batch_size)
    data_loader = _data.create_data_loader(eval_config, framework="pytorch", shuffle=False)
    return data_loader, data_loader.data_config()


@torch.no_grad()
def run_eval(model, eval_loader, accelerator, num_eval_batches=10):
    """Run evaluation on a data loader and return average losses."""
    model.eval()
    ...
    for _ in range(num_eval_batches):
        try:
            observation, actions = next(eval_iter)
        ...
        actions = actions.to(torch.float32).to(accelerator.device)
        
        loss_dict = model(observation, actions)
        
        total_loss += loss_dict['total_loss'].item()
        gaze_ce = loss_dict['gaze_cross_entropy_loss']
        gaze_mse = loss_dict['gaze_mse_loss']
        action_l = loss_dict['action_loss']
        total_gaze_ce += gaze_ce.item() if isinstance(gaze_ce, torch.Tensor) else gaze_ce
        total_gaze_mse += gaze_mse.item() if isinstance(gaze_mse, torch.Tensor) else gaze_mse
        total_action += action_l.item() if isinstance(action_l, torch.Tensor) else action_l
        num_batches += 1
    ...
    return {
        ...
        'gaze_cross_entropy_loss': total_gaze_ce / num_batches,
        'gaze_mse_loss': total_gaze_mse / num_batches,
        'action_loss': total_action / num_batches,
    }
...
def train_loop(config: _config.TrainConfig):
    ...
    loader, data_config = build_train_datasets(config)
    object.__setattr__(config, 'batch_size', original_batch_size)  # Restore original value
    
    # Build eval data loaders if configured
    # Use smaller batch size for eval to avoid OOM
    eval_batch_size = 128
    loader_eval_human = None
    loader_eval_robot = None
    if hasattr(config, 'data_eval_human') and config.data_eval_human.repo_id != "fake":
        loader_eval_human, _ = build_eval_dataset(config, config.data_eval_human, eval_batch_size=eval_batch_size)
        logging.info(f"Created eval loader for human data (batch_size={eval_batch_size})")
    if hasattr(config, 'data_eval_robot') and config.data_eval_robot.repo_id != "fake":
        loader_eval_robot, _ = build_eval_dataset(config, config.data_eval_robot, eval_batch_size=eval_batch_size)
        logging.info(f"Created eval loader for robot data (batch_size={eval_batch_size})")
        ...
        for observation, actions in loader:
            ...
            if accelerator.sync_gradients and accelerator.is_main_process and (global_step % config.log_interval == 0):
                ...
                if config.wandb_enabled and len(infos) > 0:
                    ...
                    if avg_gaze_cross_entropy_loss is not None:
                        log_payload["train/gaze_cross_entropy_loss"] = avg_gaze_cross_entropy_loss
                    if avg_gaze_mse_loss is not None:
                        log_payload["train/gaze_mse_loss"] = avg_gaze_mse_loss
                    if avg_action_loss is not None:
                        log_payload["train/action_loss"] = avg_action_loss
                    
                    # Run evaluation at log intervals
                    if loader_eval_human is not None:
                        eval_results_human = run_eval(model, loader_eval_human, accelerator)
                        if eval_results_human is not None:
                            log_payload["eval_human/loss"] = eval_results_human['loss']
                            log_payload["eval_human/gaze_cross_entropy_loss"] = eval_results_human['gaze_cross_entropy_loss']
                            log_payload["eval_human/gaze_mse_loss"] = eval_results_human['gaze_mse_loss']
                            log_payload["eval_human/action_loss"] = eval_results_human['action_loss']
                            logging.info(f"Eval human: loss={eval_results_human['loss']:.4f}, gaze_ce={eval_results_human['gaze_cross_entropy_loss']:.4f}")
                    
                    if loader_eval_robot is not None:
                        eval_results_robot = run_eval(model, loader_eval_robot, accelerator)
                        if eval_results_robot is not None:
                            log_payload["eval_robot/loss"] = eval_results_robot['loss']
                            log_payload["eval_robot/gaze_cross_entropy_loss"] = eval_results_robot['gaze_cross_entropy_loss']
                            log_payload["eval_robot/gaze_mse_loss"] = eval_results_robot['gaze_mse_loss']
                            log_payload["eval_robot/action_loss"] = eval_results_robot['action_loss']
                            logging.info(f"Eval robot: loss={eval_results_robot['loss']:.4f}, gaze_ce={eval_results_robot['gaze_cross_entropy_loss']:.4f}")
