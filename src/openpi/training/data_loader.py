...
from openpi.training.droid_rlds_dataset import DroidRldsDataset, RobotwinRldsDataset, HumanRldsDataset
...
def create_rlds_dataset(
    ...
    *,
    ...
    is_robotwin = data_config.dataset_type == "robotwin"
    is_human = data_config.dataset_type == "human"

    if is_robotwin:
        logging.info("using RobotwinRldsDataset")
        return RobotwinRldsDataset(
            data_dir=data_config.rlds_data_dir,
            batch_size=batch_size,
            shuffle=shuffle,
            action_chunk_size=action_horizon,
            down_sample=data_config.down_sample,
            dataset_name=data_config.repo_id,
            filter_dict_path=data_config.filter_dict_path,
        )
    elif is_human:
        logging.info("using HumanRldsDataset")
        return HumanRldsDataset(
            data_dir=data_config.rlds_data_dir,
            batch_size=batch_size,
            shuffle=shuffle,
            action_chunk_size=action_horizon,
            ...
            dataset_name=data_config.repo_id,
            filter_dict_path=data_config.filter_dict_path,
            dataset_weights=data_config.dataset_weights,
        )
        ...
        dataset_name = getattr(data_config, 'repo_id', 'generic_dataset')
        ...
        if hasattr(data_config, 'repack_transforms') and data_config.repack_transforms:
            for transform in data_config.repack_transforms.inputs:
                if hasattr(transform, 'mapping'):
                    field_mapping = transform.mapping
                    break
        ...
        for src_key, dst_key in field_mapping.items():
            if "action" in dst_key.lower():
                action_keys.append(src_key)
            elif "image" in dst_key.lower():
                image_keys.append(src_key)
            elif "state" in dst_key.lower() or "position" in dst_key.lower():
                state_keys.append(src_key)
            elif "prompt" in dst_key.lower() or "instruction" in dst_key.lower():
                instruction_keys.append(src_key)
        ...
        if not instruction_keys:
            instruction_keys = ["prompt"]
        
        is_robotwin = getattr(data_config, 'is_robotwin_dataset', False)
        auto_detect = getattr(data_config, 'auto_detect_fields', True)
        
        return GenericRldsDataset(
            data_dir=data_config.rlds_data_dir,
            batch_size=batch_size,
            shuffle=shuffle,
            action_chunk_size=action_horizon,
            ...
            is_robotwin=is_robotwin,
            auto_detect_fields=auto_detect,
        )
