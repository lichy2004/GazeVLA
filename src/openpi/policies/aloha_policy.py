...
class AlohaInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # Preserve gaze and mask fields if present (for RobotWin dataset)
        if "gaze" in data:
            inputs["gaze"] = data["gaze"]
        if "action_mask" in data:
            inputs["action_mask"] = data["action_mask"]
        if "gaze_mask" in data:
            inputs["gaze_mask"] = data["gaze_mask"]
...
@dataclasses.dataclass(frozen=True)
class LFAOutputs(transforms.DataTransformFn):
    """LFA专用输出转换，保留完整动作维度"""
